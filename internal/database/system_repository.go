...
// Description: Repository for star systems, planets, and jump route management.
//              Handles universe geography and navigation data.
// Version: 1.1.0
...
// System and planet errors.
var (
	// ErrSystemNotFound indicates the requested star system does not exist.
	ErrSystemNotFound = errors.New("system not found")

	// ErrPlanetNotFound indicates the requested planet does not exist.
	ErrPlanetNotFound = errors.New("planet not found")
)

// SystemRepository handles all database operations for star systems and planets.
//
// This repository manages the game universe geography:
//   - Star systems with positions, tech levels, and governments
//   - Planets within systems with services and populations
//   - Jump routes (connections between systems)
//   - Bulk operations for universe generation
//
// Data model:
//   - Star systems have x/y coordinates in 2D space
//   - Systems connect via bidirectional jump routes
//   - Each system has 0+ planets
//   - Planets inherit tech level from their system
//
// Thread-safety:
...
//
// Jump routes allow players to travel between systems. Routes are always
// bidirectional (if you can jump A→B, you can jump B→A). This method
// creates both directions atomically.
//
// Uses ON CONFLICT DO NOTHING to safely handle duplicate route creation
// (idempotent operation).
//
// Parameters:
//   - ctx: Context for timeout and cancellation
//   - systemA: First system UUID
//   - systemB: Second system UUID
//
// Returns:
//   - error: Database error (rare)
//
// Thread-safety:
//   - Safe for concurrent route creation
//   - Duplicate routes are silently ignored
...
//
// This method is optimized for universe generation, creating hundreds or
// thousands of systems efficiently. Uses a prepared statement within a
// transaction for maximum performance.
//
// All systems are created atomically - if any insert fails, all are rolled back.
//
// Parameters:
//   - ctx: Context for timeout and cancellation
//   - systems: Slice of systems to create (generated by universe generator)
//
// Returns:
//   - error: Database error (transaction rolls back on failure)
//
// Thread-safety:
//   - Safe to call concurrently (each call uses its own transaction)
//
// Performance:
//   - Can create 1000+ systems in < 1 second
//   - Uses prepared statement to avoid repeated SQL parsing
...
//
// Optimized for universe generation. Creates bidirectional routes efficiently
// using prepared statements. Routes are created atomically.
//
// Parameters:
//   - ctx: Context for timeout and cancellation
//   - routes: Slice of [systemA, systemB] UUID pairs
//
// Returns:
//   - error: Database error (rolls back on failure)
//
// Performance:
//   - Can create 1000+ routes in < 1 second
