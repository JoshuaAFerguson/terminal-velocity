...
type Server struct {
	...
	// Server-wide presence. The space view reads GetPlayersInSystem
...
//
// Request Handling:
//   - "pty-req": Accepted (BubbleTea needs a PTY for TUI rendering); window size parsed
//   - "shell": Accepted and starts anonymous session; window-change then forwarded
//   - All others: Rejected
//
// Thread Safety:
...
//   - requests: Remaining SSH request stream (for window-change forwarding)
//   - initialSize: Starting PTY dimensions from the initial pty-req
//
// Blocking Behavior:
